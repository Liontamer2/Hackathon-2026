...
class Tile:
    ...
    def draw(self, screen, font, alpha=255):
        ...
        bg = (255, 255, 255, alpha) if not self.dragging else (200, 200, 200, alpha)
        pygame.draw.rect(surf, bg, (0, 0, TILE_WIDTH, TILE_HEIGHT), border_radius=5)
        pygame.draw.rect(surf, (0, 0, 0, alpha), (0, 0, TILE_WIDTH, TILE_HEIGHT), 2, border_radius=5)
        text = font.render(str(self.number), True, list(self.color_val) + [alpha])
        surf.blit(text, text.get_rect(center=(TILE_WIDTH // 2, TILE_HEIGHT // 2)))
...
def main():
    ...
    def reset_logic():
        ...
        for t in tiles_in_play:
            if t.owner == 0:
                is_old = any(t.number == g[0] and t.color_val == g[1] and t.rect.topleft == (g[2], g[3]) for g in ghost_data)
                if not is_old: t.owner = current_player
        # Re-position tiles that were already on board
        for g in ghost_data:
            t = next((x for x in tiles_in_play if x.number == g[0] and x.color_val == g[1] and x.owner != 0), None)
            if t: t.owner = 0; t.rect.topleft = (g[2], g[3])
    ...
    for p in [1, 2]:
        ...
        sort_hand(p)
    ghost_data = [(t.number, t.color_val, t.rect.x, t.rect.y) for t in tiles_in_play if t.owner == 0]
    ...
    while running:
        for event in pygame.event.get():
            ...
            if event.type == pygame.MOUSEBUTTONDOWN:
                ...
                if btns["draw"].collidepoint(event.pos) and pool:
                    reset_logic()
                    n, c = pool.pop(); tiles_in_play.append(Tile(n, c, 50, RACK_START_Y, current_player))
                    current_player = 3 - current_player
                    sort_hand(current_player); ghost_data = [(t.number, t.color_val, t.rect.x, t.rect.y) for t in tiles_in_play if t.owner == 0]
                ...
                if btns["pass"].collidepoint(event.pos):
                    ...
                    new_pts = sum(t.number for t in tiles_in_play if t.owner == 0 and not any(t.number == g[0] and t.color_val == g[1] and t.rect.topleft == (g[2], g[3]) for g in ghost_data))
                    ...
                    if not is_board_valid:
                        ...
                        if new_pts >= 30: player_initials[current_player] = True
            ...
            if event.type == pygame.MOUSEMOTION and selected_tile:
                selected_tile.rect.topleft = (pygame.mouse.get_pos()[0] + off_x, pygame.mouse.get_pos()[1] + off_y)

        # Rendering
        screen.fill(BG_COLOR)
        pygame.draw.rect(screen, (20, 80, 20), (0, BOARD_BOUNDARY, SCREEN_WIDTH, 350))
        for g in ghost_data: Tile(g[0], g[1], g[2], g[3], 0).draw(screen, font, 60)
        
        # UI Elements
        p_col = (255, 100, 100) if current_player == 1 else (100, 100, 255)
        indicator_rect = pygame.Rect(SCREEN_WIDTH - 240, 20, 220, 95)
        pygame.draw.rect(screen, (0, 0, 0, 100), indicator_rect, border_radius=10)
        screen.blit(big_font.render(f"P{current_player} TURN", True, p_col), (SCREEN_WIDTH-230, 30))
        
        for i in [1, 2]:
            label = "30+ OK" if player_initials[i] else "Need 30"
            color = (0, 255, 0) if player_initials[i] else (200, 200, 200)
            screen.blit(font.render(f"P{i}: {label}", True, color), (SCREEN_WIDTH-230, 75 + (i-1)*20))
        ...
        for t in tiles_in_play:
            if t.owner in [0, current_player]: t.draw(screen, font)
        pygame.display.flip()
        clock.tick(60)
    pygame.quit()
